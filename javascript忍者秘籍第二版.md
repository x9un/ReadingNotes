...
1. 函数是第一类对象
+ 第一类对象的特点:能作为参数传入函数
+ 回调:建立的函数被其他函数在某个时间点再回来调用

2. 函数作为对象的乐趣
+ 存储函数:使用一个对象作为缓存
+ 自记忆函数:内部构建一个结果缓存保存计算结果

3. 函数定义
+ 函数定义，箭头函数，函数构造函数，生成器函数
+ 函数声明
```js
...
+ 剩余参数以 `...`做前缀，数组
+ 最后一个参数才能是剩余参数
+ 默认参数(ES6),创建默认参数的方式是为形参赋值

## chapter 4 理解函数调用
### questions
1. 为什么 this 参数表示函数上下文
2. 函数和方法的区别
3. 如果一个构造函数显式地返回一个对象会发生什么

### notes
1. 使用隐式函数参数
+ arguments 参数是传递给函数的所有参数集合
+ 避免把 arguments 当成数组
+ 剩余参数是真正的 Array 参数实例
+ arguments 对象作为函数参数的别名，严格模式下无法使用
+ this 参数代表函数调用相关联的对象，因此通常称为函数上下文
+ this 不仅由定义函数的方式和位置决定，还受函数调用方式的影响

2. 函数调用
+ 作为函数直接被调用:  
this 指全局上下文(window 对象) - 非严格模式
this undefined - 严格模式
+ 作为方法被调用:
函数作为某对象的方法被调用时，该对象为 this，并在函数内部可以通过参数访问到
+ 作为构造函数调用:
使用 new  
+ 构造函数的目的是创建一个新对象并进行初始化设置，然后将其作为构造函数的返回值

3. 构造函数返回值
+ 如果构造函数返回一个对象，则该对象将作为整个表达式的值返回，传入构造函数的 this 将被丢弃
+ 如果构造函数返回非对象类型，则忽略返回值，返回新创建的对象
+ 构造函数以描述所构造对象的名词命名并大写字母开头

4. 使用 apply 和 call 显示指定任何对象作为函数上下文
+ 传入 apply 和 call 的第一个参数都会作为上下文
+ 无关的值使用 call,数组使用 apply

5. 使用箭头函数绕过函数上下文
+ 箭头函数没有单独的 this 值，箭头函数的 this 与声明所在的上下文相同
+ 箭头函数从定义时所在函数继承上下文

6. 使用 bind 方法
+ bind 函数创建的新函数与原始函数的函数体相同，新函数被绑定到指定对象上

## chapter 5 闭包和作用域
### questions
1. 一个变量或方法有几种不同的作用域?这些作用域分别是什么?
2. 如何定位标识符及其值?
3. 什么是可变变量?如何在 JavaScript 中定义可变变量?
...
2. 使用闭包模拟私有变量
3. 回调函数
+ 指的是在将来不确定的某一时刻异步调用的函数
4. 通过执行上下文来跟踪代码
+ 调用栈
+ 箭头函数在执行栈调用时才进行创建 
5. 无论何时调用函数，都会创建一个新的执行环境并推入调用栈。还会创建一个与之关联的词法环境
6. 变量类型 var let(ES6) const(ES6)
+ var:距离最近的函数内部||全局词法环境，不关注块级作用域
+ let&const:在最近的词法环境中定义变量
7. 在词法环境中注册标识符:
过程:
当前环境是函数环境? -> 创建 arguments 对象和函数参数  
当前环境是函数环境|全局环境? -> 注册其他函数外面的函数声明  
当前环境是块级作用域? -> 注册当前块作用域 let const 声明的变量 ： 注册函数作用域外面 var  let 声明的对象，块作用域外面 const 声明的对象  
8. 可以在函数声明之前调用函数
